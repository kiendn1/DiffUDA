...
def load_data(args, data_folder, batch_size, train, num_workers=4, weight_sampler=False, use_fixmatch=False,partial=False, folder_src=None, is_source=False, gendata_dir=None, **kwargs):
    ...
    if args.datasets == "digits":
        ...
        data = ImageFolder(root=data_folder, transform=transformation, is_source=is_source, gendata_dir=gendata_dir)
    ...
    if weight_sampler:
        # domain_name = data_folder.split('/')[-1]
        source_dataset = datasets.ImageFolder(root=folder_src, transform=transformation)
        weights = get_classes_weight(data, source_dataset)
    else:
        weights = None
...
def get_classes_weight(dataset, source_dataset):
    src_y = np.array(source_dataset.targets)
    y = np.array(dataset.targets)

    class_sample_count = np.array(
        [len(np.where(src_y == t)[0]) for t in np.unique(src_y)])
...
def get_data_loader(dataset, batch_size, shuffle=True, drop_last=False, num_workers=0, infinite_data_loader=False,weights=None, **kwargs):
    if not infinite_data_loader:
        ...
        return InfiniteDataLoader(dataset, batch_size=batch_size, shuffle=shuffle, drop_last=drop_last, num_workers=num_workers, weights=weights, **kwargs)
